...
function createRealApi(driverError: Error) {
  ...
  const api = new OrchestratorApi(
    async () => driver as any,
    logger,
    // Fork-specific: `OrchestratorApi` takes an `EventEmitterInterface` as its
    // third constructor argument (see `@cubejs-backend/event-emitter`), ahead
    // of upstream's `options`. Not exercised by this test, so a bare stub.
