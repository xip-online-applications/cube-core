...
describe('API Gateway', () => {
  ...
  test('meta endpoint to get schema information', async () => {
    ...
    // The fork always serves the extended meta payload from plain /v1/meta (see
    // gateway.ts's "always return extended meta call" fix), so `sql` is present
    // here just like it is on the explicit ?extended request below.
