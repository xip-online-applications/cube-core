...
// `iStartsWith` is case-insensitive via `UPPER()` + `LIKE`, not native
// `ILIKE` (unlike `contains`/`startsWith`/`endsWith`), matching the
// JS-side `BaseFilter#iStartsWithWhere` semantics.
#[test]
fn test_i_starts_with_filter() {
    let result = build(indoc! {"
        filters:
          - dimension: visitors.source
    ...
    assert_filter(
        &result,
        r#"((UPPER("visitors".source) LIKE UPPER($_0_$)|| '%'))"#,
        &["goo"],
    );
}
